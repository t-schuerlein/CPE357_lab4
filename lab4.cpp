...
int main(int argc, char *argv[])
{
    ...
    clock_t a = clock();
    float init = (float) a/ CLOCKS_PER_SEC ;
    ...
    if (isParallel == 0)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < piWidth; x++)
            {
                ...
                result[(x * 3) + 0 + y * width] = g;
                result[(x * 3) + 1 + y * width] = b;
                result[(x * 3) + 2 + y * width] = r;
            }
        }
        // result = idata;
    } // else parallel = 0
    ...
    a = clock();
    cout <<  ((float) a / CLOCKS_PER_SEC - init ) /10.0 << endl;
